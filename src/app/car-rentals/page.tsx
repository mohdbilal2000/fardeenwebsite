...
import { Users, Snowflake, MessageCircle, Star, Shield, Plane } from "lucide-react";
...
// "Why choose this segment" copy shown when a category is expanded.
const CATEGORY_CONTEXT: Record<CarType, { tagline: string; why: string }> = {
  sedan: {
    tagline: "Comfort Travel",
    why: "Our sedans are the smart everyday choice — fuel-efficient, easy to park around busy monuments, and comfortable on intercity highways. Ideal for couples, solo travelers, and short transfers where you want comfort without the bulk.",
  },
  suv: {
...
function VehicleCard({ car, featured = false }: { car: CarRental; featured?: boolean }) {
  ...
  const href = getWhatsAppCarLink(
    car.name,
    `${car.routes[0].from} to ${car.routes[0].to}`,
  );
  return (
    <a
      href={href}
    ...
    >
      ...
      {/* Top-left tag */}
      {car.tag && (
        <span className="absolute top-3 left-3 inline-flex items-center gap-1 text-[10px] sm:text-[11px] font-bold text-amber-50 bg-amber-500/85 backdrop-blur-sm px-2.5 py-1 rounded-full uppercase tracking-wide">
      ...
      <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
        ...
        {car.tagline && (
          <p className="text-[12px] sm:text-sm text-amber-200/90 mt-0.5">
            {car.tagline}
          </p>
        )}
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          <span className="text-xs text-gray-300">
            Tap to reserve on WhatsApp
          </span>
          <span className="inline-flex items-center gap-1.5 bg-white group-hover:bg-amber-50 text-gray-900 text-sm font-semibold px-4 py-2 rounded-full transition-colors whitespace-nowrap">
    ...
    <>
        ...
        <section className="py-12 sm:py-16">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8 sm:mb-10">
              <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-widest text-amber-600">
              ...
              <p className="text-sm text-gray-500 mt-2 max-w-xl">
                The vehicles 80% of our international guests pick. Tap any card to
      ...
      <section className="py-12 sm:py-16 bg-gray-50/60 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="border-t border-gray-200/80 bg-white rounded-2xl overflow-hidden border border-gray-200/80">
            {CAR_TYPES.map((cat, i) => {
              const cars = carRentals.filter((c) => c.type === cat.key);
              if (cars.length === 0) return null;
              const isOpen = openCategory === cat.key;
              const ctx = CATEGORY_CONTEXT[cat.key];
              return (
                <div
                  key={cat.key}
                ...
                >
                  <button
                    ...
                    className="w-full flex items-center justify-between gap-4 px-5 sm:px-7 py-5 sm:py-6 text-left hover:bg-gray-50/80 transition-colors"
                  >
                    <div>
                      <h3 className="font-display text-lg sm:text-xl font-bold text-gray-900">
                        {cat.label} <span className="text-gray-400 font-normal">— {ctx.tagline}</span>
                      </h3>
                    ...
                    <span
                      className={`shrink-0 w-9 h-9 rounded-full border flex items-center justify-center text-xl font-light transition-all ${
                        isOpen
                          ...
                          : "text-amber-600 border-gray-200"
                      }`}
                    >
                      {isOpen ? "−" : "+"}
                    </span>
                  ...
                  {isOpen && (
                    <div className="px-5 sm:px-7 pb-8 animate-fade-in">
                      <div className="bg-amber-50/60 border border-amber-100 rounded-xl p-4 sm:p-5 mb-6">
